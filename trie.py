...
# This trie implementation comes from the gdesklets project
# released under GPL, I assume v2.
...
class _TrieNode(object):
    ...
    def insert(self, data, value):
        """
        >>> c = _TrieNode()
        >>> c.insert( list( "sentence" ), "sentence" )
...
if (__name__ == "__main__"):
    ...
    print "retrieving list for \"w?ll*\"" 
    print a, len(a)

    for word in text.split():
        t.remove(word)
        print "removed %s from index, size %s"%( word, t.get_size() )

    for word in keys:
        t.insert(list(word), word)
        print "inserting %s for %s, size: %s"% ( list( word ), word, t.get_size() )

    a = t.retrieve(list("*"))
    print "result for %s: %s (length: %s)"%( "*", a, len(a) )
